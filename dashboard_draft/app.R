...
#' *IMPORTANT*
# ALWAYS run the following BEFORE a branch is about to merge back into main for
  # GitHub
...
load('seafood_trade_data_munge_08_12_26.RData')
...
calculate_supply_metrics <- function(species, region, units = NULL, nominal = F) {
  ...
  data <- summarize_yr_spp(species, region, units = units, nominal = nominal) %>%
    ...
    mutate(APPARENT_SUPPLY = (COM_VOLUME_T - EXP_ROUND_VOLUME_T) + IMP_ROUND_VOLUME_T,
           APPARENT_SUPPLY_REL_US_PROD = APPARENT_SUPPLY / COM_VOLUME_T,
           UNEXPORTED_US_PROD_REL_APPARENT_SUPPLY = 
             abs(COM_VOLUME_T - EXP_ROUND_VOLUME_T) / APPARENT_SUPPLY,
           SPECIES = species) 
...
ui <- page_fluid(
  ...
  sidebar = sidebar(
    ...
    input_switch('inflation', 'Inflation-Adjusted (Real 2024 USD)', value = T),
    ...
    navset_tab(
      ...
      nav_panel(
        ...
        navset_card_pill(
          ...
          nav_panel(title = h6('Regional Consolidation'),
                    htmlOutput('region')),
...
server <- function(input, output, session) {
  ...
  output$download_landings <- downloadHandler(
    ...
    content = function(con) {
      ...
      final_sheet <- list('Data' = com_landings, 'Metadata' = metadata)
      write.xlsx(final_sheet, con)
    }
  )
    ...
    content = function(fname) {
      ...
      if (!is.character(try(trade_df_full()))) {
        ...
        colnames(metadata) <- c('Date Accessed', 'Time Accessed',
                                ...
                                'Region', 'Units', 'Inflation-Adjusted')
        metadata <- pivot_longer(metadata, cols = colnames(metadata))
        colnames(metadata) <- c('', '')
